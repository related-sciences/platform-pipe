...
class EvidencePipelineSuite extends FunSuite with SparkSessionWrapper with DataFrameComparison {
  ...
  test("evidence pipeline results") {
    ...
    assert(df.columns.length > 3, s"Evidence json did not parse into Spark schema correctly (columns found = ${df.columns}")
    Seq("disease", "target", "type").foreach {
      c => assert(df.columns.contains(c), s"Column $c not found in parsed evidence")
    }
    ...
    val getIdCt = (ids: Seq[String]) => df.filter($"target.id".isin(ids:_*))
    ...
    assertResult(0, "Non-reference alt genes not mapped")(getIdCt(Seq("ENSG00000223532", "ENSG00000225845")))
    assertResult(2, "Reference genes not mapped")(getIdCt(Seq("ENSG00000234745", "ENSG00000204290")))
